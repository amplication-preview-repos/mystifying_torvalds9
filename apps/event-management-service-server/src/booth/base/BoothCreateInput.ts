/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import {
  IsString,
  MaxLength,
  IsOptional,
  ValidateNested,
} from "class-validator";
import { EventWhereUniqueInput } from "../../event/base/EventWhereUniqueInput";
import { Type } from "class-transformer";
import { ExhibitorWhereUniqueInput } from "../../exhibitor/base/ExhibitorWhereUniqueInput";
import { ExhibitorCreateNestedManyWithoutBoothsInput } from "./ExhibitorCreateNestedManyWithoutBoothsInput";

@InputType()
class BoothCreateInput {
  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  description?: string | null;

  @ApiProperty({
    required: false,
    type: () => EventWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => EventWhereUniqueInput)
  @IsOptional()
  @Field(() => EventWhereUniqueInput, {
    nullable: true,
  })
  event?: EventWhereUniqueInput | null;

  @ApiProperty({
    required: false,
    type: () => ExhibitorWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => ExhibitorWhereUniqueInput)
  @IsOptional()
  @Field(() => ExhibitorWhereUniqueInput, {
    nullable: true,
  })
  exhibitor?: ExhibitorWhereUniqueInput | null;

  @ApiProperty({
    required: false,
    type: () => ExhibitorCreateNestedManyWithoutBoothsInput,
  })
  @ValidateNested()
  @Type(() => ExhibitorCreateNestedManyWithoutBoothsInput)
  @IsOptional()
  @Field(() => ExhibitorCreateNestedManyWithoutBoothsInput, {
    nullable: true,
  })
  exhibitors?: ExhibitorCreateNestedManyWithoutBoothsInput;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  name?: string | null;
}

export { BoothCreateInput as BoothCreateInput };
